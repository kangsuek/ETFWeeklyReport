...
from app.models import ETF, PriceData, ETFMetrics
...
from app.services.trading_flow_collector import TradingFlowCollectorMixin
...
class ETFDataCollector(TradingFlowCollectorMixin):
    """Service for collecting ETF/Stock data from various sources

    가격 수집·조회와 전체 수집 오케스트레이션을 담당한다.
    매매동향은 TradingFlowCollectorMixin, 지표 계산은 metrics_service로 분리됨.
    """
    ...
    def get_etf_metrics(self, ticker: str) -> ETFMetrics:
        """종목 주요 지표 조회 — 계산은 metrics_service로 일원화 (위임)"""
        return metrics_service.get_etf_metrics(ticker)
