...
# =============================================================================
# 감지기 판정 파라미터. 값 변경은 신호 빈도·확정률에 직접 영향(Phase 4 백테스트로 조정).
# SIGNAL_MIN_DATA_DAYS는 갭 보충 헬퍼(Phase 1.1)가 먼저 필요로 해 선행 정의됐다.
...
"""돌파선·평균 거래량 산출 기간 (거래일). Donchian 20일 채널 상단을 돌파선으로 사용."""

SIGNAL_VOL_MULT = 2.0
"""거래량 배수 임계. 돌파일 거래량이 직전 LOOKBACK 평균 대비 이 배수 이상이어야 유효."""

SIGNAL_CANDLE_POS_MIN = 0.6
"""캔들 위치 하한. (종가-저가)/(고가-저가)가 이 값 이상이어야 상단 마감으로 인정(0~1)."""

SIGNAL_OVERHEAT_5D = 25.0
"""과열 제외 기준 (%). 최근 5일 상승률이 이 값을 넘으면 추격 과열로 보고 신호 제외."""

# --- LV2 확정/실패 판정 파라미터 (§2-3, §2-4) ---

SIGNAL_CONFIRM_WINDOW = 15
"""확정 대기 기간 (거래일). LV1 발생 후 이 기간 내 확정되지 않으면 만료(expired)."""

SIGNAL_RETEST_NEAR = 0.02
"""재시험 접근 허용 폭 (±2%). 돌파선 부근 되돌림 후 반등을 재시험 확정(C1)으로 판정."""

SIGNAL_FAIL_FLOOR = 0.97
"""실패 판정선 (돌파선 대비). 종가가 돌파선×이 값 아래로 마감하면 실패(failed)."""

SIGNAL_HOLD_DAYS = 3
"""연속 유지 확정 일수. 돌파선 위에서 이 일수만큼 연속 유지되면 확정(C2)."""

SIGNAL_COOLDOWN_DAYS = 20
"""동일 종목 LV2 재알림 금지 기간 (거래일). 직전 확정으로부터 이 기간 내 재확정은 알림 억제."""

SIGNAL_BATCH_SCAN_MAX = 50
"""배치 흐름 점검(조건검색 결과 등) 1회 최대 종목 수.

종목당 가격·수급 이력을 네이버에서 즉시 수집하므로(스크래핑 rate limit 0.3초/요청,
종목당 대략 2초) 상한을 두어 응답 시간과 대상 서버 부하를 통제한다.
"""

SIGNAL_ALERT_FRESH_DAYS = 10
"""확정 알림을 발신할 최대 신선도 (거래일).

확정일이 최신 데이터로부터 이 일수보다 오래됐으면 상태(signal_events)만 기록하고
사용자 알림(alert_history)은 억제한다. 최초 스캔·장기 미기동 따라잡기가 소급 재생하며
수개월 전 확정을 '새 알림'처럼 띄우는 것을 방지한다(설계 §3-4: 오래된 확정은 이력만).
"""
