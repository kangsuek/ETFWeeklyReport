...
배포 실패의 가장 흔한 원인 4가지: **`API_KEY` 미설정**(503), **`VITE_API_BASE_URL` 미설정**(404), **`CORS_ORIGINS` 미설정**(CORS 에러), **초기 데이터 수집 누락**(빈 배열 응답). 증상별 해결은 [트러블슈팅](#트러블슈팅) 참고.
...
> ⚠️ **단일 프로세스 전제**: 백엔드의 인메모리 캐시(`utils/cache.py`)와
> slowapi rate limit은 프로세스 로컬입니다. `--workers 2` 이상 또는 gunicorn
> 멀티 워커로 실행하면 캐시 무효화(X-No-Cache)와 rate limit이 워커별로
> 갈라지므로, 워커는 반드시 1개로 유지하세요 (Mac 앱은 단일 uvicorn이라 무관).
...
| 증상 | 원인 | 해결 |
|------|------|------|
| 데이터 수집/설정 API → **503** | Render 환경에서 `API_KEY` 미설정 (백엔드 로그: "프로덕션 환경에서 API_KEY가 설정되지 않았습니다") | Backend 환경 변수 `API_KEY` 설정 후 재배포 |
| API 요청 **404** — Network 탭에 요청 URL이 프론트 도메인(`...frontend.onrender.com/api/...`) | `VITE_API_BASE_URL` 미설정 → `/api` 상대경로 사용 | Frontend 환경 변수 설정 후 **재배포** (빌드 시점 삽입) |
| 브라우저 콘솔 **CORS 에러** | `CORS_ORIGINS`에 프론트 도메인 없음 | `CORS_ORIGINS=https://...frontend.onrender.com` (끝에 `/` 없이) 설정 후 백엔드 재배포 |
| 모든 API 응답이 빈 배열 `[]` | 데이터 수집 미실행 | [배포 후 초기 데이터 수집](#배포-후-초기-데이터-수집) 참고 |
| DB 테이블 없음 에러 | `startCommand`가 `uvicorn`만 실행 | Start Command를 `cd backend && bash render-start.sh`로 변경 후 재배포 |
| 첫 요청 30초 이상 지연 | 무료 플랜 슬립 모드 (15분 무요청 시) | UptimeRobot 등으로 `/api/health` 5분 간격 Keep-Alive |
