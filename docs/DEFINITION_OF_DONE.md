...
#### 기능 요구사항
- [ ] 6개 종목 모두 데이터 수집 성공 (ETF 4개 + 주식 2개)
- [ ] 투자자별 매매 동향 데이터 수집
- [ ] 뉴스 스크래핑 동작
- [ ] 스케줄러 정상 작동
...
각 Phase 완료 시 확인:
- `pytest tests/ -v --cov=app` (백엔드)
- `npm test -- --coverage` (프론트엔드)
- 린트 체크 통과
...
1. Phase DoD 확인 → 2. 기능 구현 + 테스트 작성 → 3. 100% 통과 → 4. 문서 업데이트 → 5. 다음 Phase

---

## 🎯 요약

**핵심 원칙**: 테스트 100% 통과 후 다음 단계 진행, 모든 Acceptance Criteria 충족 필수
