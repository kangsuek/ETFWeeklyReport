...
├── database.py          # DB 연결 풀 (SQLite 전용 — 이 브랜치)
...
├── services/
│   ├── naver_stock_api.py        # 가격 수집 (Naver Finance)
│   ├── trading_flow_collector.py # 매매동향 수집
│   ├── data_collector.py         # 수집 오케스트레이션·지표 조회
│   ├── metrics_service.py        # 지표 산식 정본 (docs/METRICS_SPEC.md)
│   ├── intraday_collector.py / intraday_service.py  # 분봉 수집·조회
│   ├── news_scraper.py           # 뉴스 수집 (Naver Search API)
│   ├── news_analyzer.py          # 뉴스 감성·토픽 분석
...
│   ├── summary_service.py        # 배치 요약 (대시보드)
│   ├── etf_fundamentals_collector.py / stock_fundamentals_collector.py  # 펀더멘털 수집
...
│   ├── etf/             # ETFCard, ETFHeader, ETFCharts, ETFBasicInfo, ETFKeyMetrics, ETFFundamentalInfo, StrategySummary, InsightSummary, PriceTargetPanel, PriceTable
...
| 프로덕션  | Render 등   | Vercel/Netlify 등 | SQLite (이 브랜치는 SQLite 전용) |
