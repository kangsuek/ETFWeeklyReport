...
│    └── FastAPI(Uvicorn) → localhost:18001 (Windows 전용 포트)   │
├─────────────────────────────────────────────────────────────┤
│  데이터                                                       │
...
- **프론트**: `VITE_API_BASE_URL=http://localhost:18001/api` 로 빌드 (Mac 앱 18000과 구분, [BRANCHES.md](BRANCHES.md)#app-ports-mac-vs-windows 참고).
...
| 2.2 | 진입점 스크립트 | `backend/win_main.py` 또는 기존 `uvicorn app.main:app --host 127.0.0.1 --port 18001` 를 한 번 감싼 배치/스크립트. |
...
| 2.4 | CORS | Electron에서 로드하는 프론트 오리진(`file://` 또는 `http://localhost:18001`)을 `Config.CORS_ORIGINS`에 추가. |
...
| 3.2 | 준비 대기 | 백엔드 기동 후 `http://127.0.0.1:18001/api/health` 등으로 헬스체크, 성공 시에만 `BrowserWindow.loadURL` 또는 `loadFile`. |
...
| 순서 | 작업 | 상세 |
|------|------|------|
| 4.1 | 프론트엔드 빌드 | `ELECTRON_BUILD=1 VITE_API_BASE_URL=http://localhost:18001/api npm run build` (Windows 전용 포트 18001). |
...
| 5.3 | 포트 충돌 | 18001 사용 중이면 재시도 또는 사용자에게 “다른 인스턴스가 실행 중” 메시지. |
...
| **API Base URL** | 빌드 시 `http://localhost:18001/api` 고정 (Mac 18000과 분리) |
