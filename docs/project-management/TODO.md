...
- [x] 백엔드: 엔드포인트별 차등 TTL 적용
  - 정적 데이터(종목 목록): 5분
  - 빠르게 변하는 데이터(가격, 매매동향, 배치 요약): 30초
  - 천천히 변하는 데이터(뉴스, 지표, 비교): 1분
  - 상태 정보: 10초
  - 통계 정보: 1분
- [x] 캐시 무효화 로직 추가 (데이터 수집 후)
  - `collect-all`, `backfill` 엔드포인트에서 자동 캐시 클리어
  - 뉴스 수집 시 해당 종목 캐시 패턴 무효화
- [x] 프론트엔드: TanStack Query 캐시 설정 최적화
  - 백엔드 TTL과 일치하는 staleTime 적용
  - gcTime: 10분 (메모리 유지 시간)
  - Dashboard, ETFDetail, Comparison 페이지 최적화
- [x] 캐시 적중률 모니터링 구현
  - Settings 페이지에 캐시 통계 표시
  - 적중률, 히트/미스, 캐시 크기 실시간 모니터링
  - 10초마다 자동 갱신
