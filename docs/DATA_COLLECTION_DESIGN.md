...
| 투자자 매매 동향 | ✅ 완료 | Phase 2 | 개인/기관/외국인 순매수 (실제 스크래핑) |
| 뉴스 데이터 | ⚠️ Mock | Phase 2 | **Mock 구현**: 실시간 스크래핑 불가 (JS 동적 로딩) |
| 자동 스케줄링 | ✅ 완료 | Phase 2 | APScheduler로 일일/주간 자동 수집 |

### ⚠️ 뉴스 데이터 제한사항

**현재 상태**: Mock 데이터 구현
- **문제**: Naver 뉴스는 JavaScript 동적 로딩 사용
- **제한**: `requests` + `BeautifulSoup`만으로는 실시간 스크래핑 불가
- **필요**: Selenium 또는 Playwright 같은 브라우저 자동화 도구
