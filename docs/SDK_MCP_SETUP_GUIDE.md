...
백엔드는 **uv 전용**입니다 (`pip`/`python` 직접 호출 금지).

```bash
cd backend
uv venv && uv pip install -r requirements.txt
uv pip install -r requirements-dev.txt   # 개발 환경만
```

### 2-2. 환경 변수 설정

**프로젝트 루트**에 `.env` 파일을 생성합니다 (백엔드는 루트 `.env`만 읽습니다).
...
| 변수 | 필수 | 기본값 | 설명 |
|------|------|--------|------|
| `DATABASE_URL` | 아니오 | `backend/data/etf_data.db` | SQLite 파일 경로 (`sqlite:///...`, SQLite 전용) |
