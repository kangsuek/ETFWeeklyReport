...
## ✅ 진행 상태 (2026-07-03 업데이트)

| 항목 | 상태 | 비고 |
|---|---|---|
| P1-1.1 (to_thread 래핑) | **완료** | GET /news/{ticker}의 온디맨드 수집 경로도 추가 수정 |
| P1-1.2 (X-No-Cache 범위 축소) | **완료** | 티커 단위 무효화, 티커 없으면 전체 삭제 폴백 |
| P1-1.3 (lifespan 전환) | **완료** | close_connection_pool() 연결 |
| P1-1.4 (sqlite3.Error 결합 제거) | **완료** | get_db_connection에서 DatabaseException으로 변환 |
| P1-1.5 (DB 방향 결정) | **완료** | (a) SQLite 전용 확정, placeholder 잔재 제거 |
