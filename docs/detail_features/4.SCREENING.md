...
종목 발굴(스크리닝)은 **조건 기반 ETF/주식 검색**과 **테마별 탐색**을 제공하는 페이지로, `/screening` 경로에서 접근한다.
...
| 순서 | API | 용도 | 캐시 정책 |
|------|-----|------|----------|
| 1 | `GET /api/screening` | 조건 기반 종목 검색 | staleTime 30초, keepPreviousData |
| 2 | `GET /api/screening/themes` | 섹터/테마별 그룹 조회 | staleTime 5분 (테마 탐색 탭) |
| 3 | `POST /api/screening/collect-data` | 카탈로그 데이터 수집 트리거 | - |
| 4 | `GET /api/screening/collect-progress` | 수집 진행률 조회 | 폴링 2초 간격 (수집 중) |
| 5 | `POST /api/screening/cancel-collect` | 수집 중지 요청 | - |

### 요청 순서

**조건 검색 탭:**
1. 탭 진입 시 → `GET /api/screening` (필터 파라미터와 함께)
2. 필터/정렬/페이지 변경 시 → 동일 API 재호출 (`enabled: activeTab === 'search'`)

**테마 탐색 탭:**
1. 탭 진입 시 → `GET /api/screening/themes`

**데이터 수집:**
1. [데이터 수집] 클릭 → `POST /api/screening/collect-data`
2. `isCollecting=true` 설정 후 → `GET /api/screening/collect-progress` 2초 간격 폴링
3. `status=completed` → 폴링 중지, Toast 성공, screening/themes/recommendations 캐시 무효화
...
- 조건 검색 탭에서만 `GET /api/screening` 호출 (`enabled` 조건)
...
| 버튼 | 동작 |
|------|------|
| **데이터 수집** | `POST /api/screening/collect-data` 호출, 수집 중이면 disabled |
| **중지** | `POST /api/screening/cancel-collect` 호출 |
...
**진행 상태 처리:**
- `completed`: Toast 성공, screening/themes/recommendations 캐시 무효화
- `cancelled`: Toast 정보
- `error`: Toast 에러
- `idle`: UI 정리
...
| `services/api.js` | screeningApi (search, getThemes, collectData, getCollectProgress, cancelCollect) |

### 백엔드

| 파일 | API |
|------|-----|
| `routers/screening.py` | GET /api/screening, GET /api/screening/themes, GET /api/screening/recommendations, POST /api/screening/collect-data, GET /api/screening/collect-progress, POST /api/screening/cancel-collect |
