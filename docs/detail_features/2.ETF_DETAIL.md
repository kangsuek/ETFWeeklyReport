...
백엔드 InsightsService(`GET /api/etfs/{ticker}/insights`)가 생성한 인사이트를 표시.
...
- **계산은 백엔드가 정본** — 판정 기준은 [METRICS_SPEC.md](../METRICS_SPEC.md) 참조. 프론트는 표시만 담당
- 데이터 부족 시 자동 숨김
...
### 3-4. 종목 정보 + 최근 가격 카드 (ETFBasicInfo)
...
인접 섹션 (같은 리팩토링에서 추출된 컴포넌트):
- **ETFKeyMetrics** — 핵심 지표 (수익률·변동성·MDD·샤프, `GET /api/etfs/{ticker}/metrics` 표시)
...
- 최근 7일, 최대 10건
- 타임라인 형태 표시
- 분석 옵션 (`analyze: true`) — 센티먼트·토픽 분석은 백엔드 `news_analyzer.py`가 담당, 프론트는 뱃지 표시만
...
| `components/etf/ETFBasicInfo.jsx` | 종목 정보 + 최근 가격/투자 현황 카드 |
| `components/etf/ETFKeyMetrics.jsx` | 핵심 지표 (metrics API 표시) |
| `components/etf/ETFFundamentalInfo.jsx` | 펀더멘털 정보 (STOCK/ETF 구분) |
...
| `utils/technicalIndicators.js` | RSI, MACD, 지지/저항선 계산 (차트 시각화 전용 — 판정은 백엔드) |
