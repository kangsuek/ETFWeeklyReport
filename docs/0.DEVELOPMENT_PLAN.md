...
- [2단계: 경쟁력 확보](#2단계-경쟁력-확보)
  ...
  - [2-3. 종목 발굴 / 스크리닝](#2-3-종목-발굴--스크리닝--구현-완료)
...
```sql
-- ✅ 구현됨 (stock_catalog 테이블에 7개 컬럼 + 3개 인덱스 추가)
...
CREATE INDEX IF NOT EXISTS idx_stock_catalog_screening ON stock_catalog(type, is_active, weekly_return);
CREATE INDEX IF NOT EXISTS idx_stock_catalog_sector ON stock_catalog(sector, is_active);
CREATE INDEX IF NOT EXISTS idx_stock_catalog_catalog_updated ON stock_catalog(catalog_updated_at);
```
...
| # | 작업 | 상세 | 영향 범위 | 상태 |
|---|------|------|----------|------|
| 1 | **스크리닝 API** | `GET /api/screening` — 조건 필터(수익률, 수급 등), 정렬, 페이지네이션 | `routers/screening.py` (신규) | ✅ |
| 2 | **테마 탐색 API** | `GET /api/screening/themes` — 테마별 ETF 목록 (sector 기반 그룹핑) | `routers/screening.py` | ✅ |
| 3 | **추천 API** | `GET /api/screening/recommendations` — 5개 프리셋 (주간수익률/외국인/기관/거래량/하락) | `routers/screening.py` | ✅ |
| 4 | **카탈로그 데이터 수집** | `catalog_data_collector.py` 신규. Phase1(가격일괄) + Phase2(개별수급). 진행률 표시 + 취소 지원 | `services/catalog_data_collector.py` (신규) | ✅ |
| 5 | **수동 수집 트리거** | `POST /api/screening/collect-data` + 진행률 조회 + 취소 | `routers/screening.py` | ✅ |
| 6 | **스케줄러 연동** | 매일 16:00 KST 자동 수집 | `services/scheduler.py` | ✅ |
| 7 | **섹터 자동 매핑** | ETF 이름 기반 24개 카테고리 섹터 분류 | `services/catalog_data_collector.py` | ✅ |

#### 프론트엔드 작업

| # | 작업 | 상세 | 영향 범위 | 상태 |
|---|------|------|----------|------|
| 5 | **스크리닝 페이지** | `/screening` 2탭(조건검색/테마탐색). 필터 + 테이블 + 히트맵 뷰 토글 | `pages/Screening.jsx` (신규) | ✅ |
| 5-1 | **필터 패널** | 검색, 주간수익률 범위, 외국인/기관 순매수 토글, 갱신 시각 | `components/screening/ScreeningFilters.jsx` (신규) | ✅ |
| 5-2 | **결과 테이블** | 7컬럼 정렬, 종목명 클릭→Settings 추가, 페이지네이션 | `components/screening/ScreeningTable.jsx` (신규) | ✅ |
| 5-3 | **히트맵 뷰** | Recharts Treemap, 주간수익률 기반 색상, 거래량 기반 크기 | `components/screening/ScreeningHeatmap.jsx` (신규) | ✅ |
| 6 | **테마 탐색 탭** | 섹터별 카드 그리드, 카드 클릭→조건검색 탭 전환+섹터 필터 | `components/screening/ThemeExplorer.jsx` (신규) | ✅ |
| 7 | **추천 카드** | 대시보드에 3개 프리셋 카드 + "더보기" 링크 | `components/dashboard/RecommendationCards.jsx` (신규) | ✅ |
| 8 | **헤더 메뉴 추가** | 네비게이션에 "종목 발굴" (데스크톱+모바일) | `components/layout/Header.jsx` | ✅ |
| 9 | **API 서비스** | `screeningApi` (search, themes, recommendations, collectData, cancelCollect) | `services/api.js` | ✅ |
| 10 | **라우트 등록** | `/screening` lazy 로드 | `App.jsx` | ✅ |
| 11 | **대시보드 배치** | RecommendationCards: PortfolioHeatmap 아래, ETFCardGrid 위 | `pages/Dashboard.jsx` | ✅ |

#### 2-3 구현 요약

> **구현된 것**: DB 스키마(7컬럼+3인덱스), 카탈로그 데이터 수집기(2단계 수집+진행률+취소), 스크리닝 API(조건검색/테마/추천/수집+취소 6개 엔드포인트), 스케줄러 연동(매일 16:00), ETF 이름 기반 섹터 자동 매핑, 스크리닝 페이지(2탭+필터+테이블+히트맵 뷰 토글), 테마 탐색(섹터 카드 그리드), 대시보드 추천 카드, 종목명 클릭→Settings 추가 이동, 헤더 네비게이션
> **추가 구현 (계획 외)**: Treemap 히트맵 뷰, 수집 진행률 표시, 수집 중지 기능, 정렬 드롭다운(히트맵 포함)
...
| 데이터 수집 | `backend/app/services/catalog_data_collector.py` | Phase1(ETF 가격 일괄) + Phase2(개별 수급). 진행률 표시 + 취소 지원 + 섹터 자동 매핑 |
| 스크리닝 API | `backend/app/routers/screening.py` | 조건검색(GET) + 테마(GET /themes) + 추천(GET /recommendations) + 수집(POST /collect-data) + 진행률 + 취소 |
| API 등록 | `backend/app/main.py` | `/api/screening` 프리픽스로 라우터 등록 |
| 스케줄러 | `backend/app/services/scheduler.py` | 매일 16:00 KST 카탈로그 데이터 자동 수집 |
| 진행률 관리 | `backend/app/services/progress.py` | 스레드 안전 진행률 저장소 + 취소 플래그 |
| 스크리닝 페이지 | `frontend/src/pages/Screening.jsx` | 2탭(조건검색/테마탐색) + 테이블/히트맵 뷰 토글 + 정렬 드롭다운 + 수집 진행률/취소 |
| 필터 패널 | `frontend/src/components/screening/ScreeningFilters.jsx` | 검색, 주간수익률 범위, 외국인/기관 순매수 토글, 갱신 시각 |
| 결과 테이블 | `frontend/src/components/screening/ScreeningTable.jsx` | 7컬럼 정렬 + 종목명 클릭→Settings 추가 + 페이지네이션 |
| 히트맵 뷰 | `frontend/src/components/screening/ScreeningHeatmap.jsx` | Recharts Treemap, 주간수익률 색상, log-scale 거래량 크기 |
| 테마 탐색 | `frontend/src/components/screening/ThemeExplorer.jsx` | 섹터별 카드 그리드 + 카드 클릭→조건검색 탭 전환 |
| 추천 카드 | `frontend/src/components/dashboard/RecommendationCards.jsx` | 3개 프리셋 카드 + "더보기" 링크 |
