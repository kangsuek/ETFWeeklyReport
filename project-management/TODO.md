...
#### Step 2: 가격 데이터 수집 구현 (예상: 1.5시간)
- [ ] Naver Finance 스크래핑 함수 구현
  - [ ] requests + BeautifulSoup4로 HTML 파싱
  - [ ] `table.type2`에서 데이터 행 추출
  - [ ] 날짜/종가/시가/고가/저가/거래량/등락률 파싱
- [ ] 파싱 데이터 → PriceData 모델 변환
- [ ] 날짜 범위 파라미터 처리 (기본 10일치 데이터)
- [ ] 기본 에러 처리 (네트워크 오류, HTML 구조 변경)
