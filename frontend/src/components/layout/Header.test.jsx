...
describe('Header', () => {
  ...
  it('/ 경로에서 대시보드 링크가 활성화된다', () => {
    renderWithProviders(<Header />, { initialEntries: ['/'] })

    const dashboardLinks = screen.getAllByText('대시보드')
    // 데스크톱 네비게이션 링크 확인 (첫 번째)
    expect(dashboardLinks[0].closest('a').className).toContain('bg-primary-500')
    expect(dashboardLinks[0].closest('a').className).toContain('text-white')
  })

  it('/compare 경로에서 비교 링크가 활성화된다', () => {
    renderWithProviders(<Header />, { initialEntries: ['/compare'] })

    const comparisonLinks = screen.getAllByText('비교')
    // 데스크톱 네비게이션 링크 확인 (첫 번째)
    expect(comparisonLinks[0].closest('a').className).toContain('bg-primary-500')
    expect(comparisonLinks[0].closest('a').className).toContain('text-white')
  })
